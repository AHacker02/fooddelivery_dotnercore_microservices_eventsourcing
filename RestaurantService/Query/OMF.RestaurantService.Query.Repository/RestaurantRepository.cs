            ...
            string budget, string rating, string food, string distance, string cuisine)
        {
                var filteredRestaurant = from r in _database.TblRestaurant
                    where (Id == 0 || r.Id == Id)
                          ...
                          && (rating == null || r.TblRating.Average(x => Convert.ToDecimal(x.Rating)) >= Convert.ToDecimal(rating))
                    ...
                    orderby r.TblRating.Average(x=>Convert.ToDouble(x.Rating)) descending 
                    select r;
